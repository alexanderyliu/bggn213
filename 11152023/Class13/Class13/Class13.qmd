...
```{r}
sum(as.vector(metadata$dex == "control"))
```

four control cell lines.
...
unique() is to ensure no row is counted twice.
...
These fold change data is still not shown to be significant. It should not be trusted.
